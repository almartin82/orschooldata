"""
Core functions wrapping orschooldata R package via rpy2.
"""
...
def _get_pkg():
    ...
    global _pkg
    if _pkg is None:
        _pkg = importr("orschooldata")
    return _pkg


def fetch_enr(end_year: int) -> pd.DataFrame:
    """
    Fetch Oregon school enrollment data for a single year.
...
def fetch_enr_multi(end_years: list[int]) -> pd.DataFrame:
    """
    Fetch Oregon school enrollment data for multiple years.
    ...
    --------
    >>> import pyorschooldata as or_
    >>> df = or_.fetch_enr_multi([2020, 2021, 2022])
    """
    pkg = _get_pkg()
    with localconverter(robjects.default_converter + pandas2ri.converter):
