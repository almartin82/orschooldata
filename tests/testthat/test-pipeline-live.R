...
test_that("Oregon DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Oregon DOE URL and verify HTTP 200")
})
...
test_that("Can download Oregon enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Oregon enrollment file", {
  skip_if_offline()
...
test_that("orschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- orschooldata::get_available_years()
  ...
  tryCatch({
    raw <- orschooldata:::get_raw_enr(test_year)
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- orschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- orschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- orschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- orschooldata::fetch_enr(test_year, tidy = TRUE)
